...
void fexportToFile() {
	FILE *file = fopen("books.txt", "w");  // Open file in write mode
	if (file == NULL) {  // Check if file opened successfully
    	printf("Error opening file for writing.\n");
    	return;
	}

	// Loop through all the books and write each one to the file
	for (int i = 0; i < count; i++) {
    	// Write book title, author, and ISBN
    	fprintf(file, "%s\n", books[i].title);	// Write book title
    	fprintf(file, "%s\n", books[i].author);   // Write book author
    	fprintf(file, "%s\n", books[i].isbn); 	// Write book ISBN
	}
...
void fimportFromFile() {
	...
	// Read the file content into the books array
	while (fscanf(file, "%[^\n]%*c", books[count].title) == 1) {
    	fscanf(file, "%[^\n]%*c", books[count].author);   // Read author
    	fscanf(file, "%[^\n]%*c", books[count].isbn); 	// Read ISBN
